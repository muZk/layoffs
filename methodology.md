...
- Método: clasificación por reglas (regex sobre el reason text) más 58 overrides manuales para casos high-confidence.
...
- Método. Agentes paralelos extrajeron el headcount end-of-FY de los 10-K (US issuers) o el 20-F (foreign private issuers, ASML, Stone, Wix) vía SEC EDGAR. Para cada empresa calcularon el growth % en ventanas de 2 años y ajustaron por M&A cuando aplicaba (Block-Afterpay, Bill.com-Divvy/Invoice2go, ZoomInfo-Chorus.ai/RingLead, Intuit-Mailchimp, Oracle-Cerner, Cisco-Splunk, Stone-Linx, Workday-Peakon, WiseTech-multiple).
...
- Caveats finos:
  - Amazon calificó TRUE solo para el segmento corporate. Amazon no disclosa el split corporate-vs-warehouse en el 10-K. La cifra de "corporate triplicó 2019→2022" viene de leaks de The Information y de earnings calls, no de disclosure auditado.
  - Pinterest y Coinbase: el cut 2026 corrige la segunda ola de hiring (FY23→FY25 AI rebuild), no la ola pandemic original, ya absorbida en 2022-23.
  - C3.ai es un caso de AI-hype overhire (FY25 +33% apostando a demanda GenAI que no se materializó), no COVID puro.
  - Workday calificó TRUE: el revenue creció +95%, más que el headcount (+61%).
  - ASML calificó FALSE pese a +35.7% de crecimiento orgánico FY20→FY22: es ciclo de capex semiconductor (EUV/High-NA), no demanda SaaS pandémica.
- Impacto en el dataset: el bucket hire-then-fire pasó de 7 empresas / ~13k personas (~11%) a 16 empresas / ~42k personas (~36%).
- Citas SEC empresa-por-empresa están en el dataset curado (`2026-categorized.json`) y en footnotes [^33] y [^34] del newsletter.
...
- Las alternativas para auditar startups no son equivalentes: LinkedIn employee count (self-reported, sin histórico), Crunchbase (employee ranges, no exacto), SignalFire Beacon (free pero cobertura parcial), PitchBook (paid). Workforce.ai (Live Data Technologies) es estrictamente enterprise, no tiene API pública.
- Implicación práctica: las ~38 startups del dataset suman menos del 5% del headcount del wave (cuts promedio menores a 130 personas). Son cualitativamente distintas a los mega-eventos Post-IPO (death-by-funding-runway vs. unwinding de pandemic-era hiring), pero no se pueden cuantificar con el mismo rigor.
- Otras empresas no auditadas: Sama (privada, Kenia), UKG (PE-owned desde 2020), LinkedIn (subsidiaria de Microsoft sin disclosure standalone), DeepL (privada, Alemania), Kraken (privada, US). Para todas, `hire_overcorrection` queda en `null`: desconocido, no descartado.
...
Por evento auditable: la lista completa de los 160 eventos con su fuente original y su fuente de recovery está en [`sources.md`](sources.md), un mapping fila por fila para rastrear cualquier claim del análisis hasta su fuente.
...
El primer paso de clasificación aplica patrones regex sobre el reason text recuperado:
...
- Cloudflare (clasificado en su momento como direct_substitution; esa clasificación de causa fue revisada después, ver nota al inicio del documento)
...
- MercadoLibre: inicialmente `vertical_builder`, reclasificado a `token_buyer` por no cumplir el criterio estricto (no foundation models propios, no silicon, no datacenters a escala).
...
- No auditadas (Tier 2 / 3, marcadas null): ~35 empresas restantes con cuts <500 personas, privadas (Sama, UKG, DeepL, Kraken), o subsidiarias sin disclosure standalone (LinkedIn).
...
## 5. Archivos y fuente de verdad

`2026-categorized.json` (y su vista plana `2026-categorized.csv`) es el dataset curado a mano y la fuente de verdad del análisis. Cada evento pasó por clasificación por reglas más overrides manuales (46 overrides high-confidence) y, para los casos grandes, adjudicación con fuentes primarias (SEC filings, memos, coverage directa).

`2026-enriched.json` es el insumo intermedio: los reasons recuperados en la Fase 2-3 antes de la clasificación. De ahí partió el trabajo de categorización.

`sources.md` es el mapa fila por fila de empresa a fuente (original y de recovery), para rastrear cualquier claim del análisis hasta su origen.
...
- [`2026-categorized.json`](2026-categorized.json), vista estructurada con todos los campos, dataset curado a mano
...
- `hire_overcorrection` cubre 30/161 eventos (24 True, 6 False). Los demás 131 son null porque no hay data de crecimiento: no significa False, significa unknown.
- Las declaraciones de los CEOs son self-serving. Que un memo diga "AI-first" no prueba que la IA sea el motivo real. Prueba que esa es la narrativa pública que la empresa elige contar. El veredicto (`ai_claim_verdict`) audita cada claim contra los hechos en vez de darlo por bueno.
...
- Junior collapse podría tener efectos de muestra. La caída de 31% a 9% es dramática, pero el total de postings también cayó ~30%: parte del efecto podría ser composición de quién publica en GoB, no del mercado real.
...
- +4 eventos nuevos en junio: GitLab (3-jun, 350, capex_funding), Robinhood (16-jun, 290, clasificado en su momento como ai_substitution_claim con hire_overcorrection=TRUE tras auditar el 10-K; esa clasificación de causa fue revisada después, ver nota al inicio del documento), Bungie (25-jun, 400, strategic_pivot).
...
Para mantener el análisis vivo: repetir el proceso de curación (recuperar la razón pública, clasificar por reglas más revisión manual, adjudicar con fuentes primarias) cuando hay un evento significativo nuevo. La estructura del repo soporta updates incrementales sin re-trabajo.
...
- Reclasificaciones de borderline cases (revisar los overrides manuales documentados en el dataset)
